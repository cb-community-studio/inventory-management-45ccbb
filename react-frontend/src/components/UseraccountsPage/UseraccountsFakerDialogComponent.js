...
import useraccountsFakerFactory from "./useraccountsFakerFactory";
...
const UseraccountsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = useraccountsFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("useraccounts").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(UseraccountsFakerDialogComponent);
